...
Our project, as depicted in the flowchart, focuses on securing machine learning models against data poisoning attacks using a multi-layered pipeline that integrates attack simulation, detection, purification, and retraining to achieve a robust and secured model. We start with clean training data, extracting features to train a base XGBoost model, which serves as the foundation for evaluating vulnerabilities. The data is then subjected to attack simulation, where we introduce three types of poisoning: FGSM (ε=0.05) for subtle gradient-based perturbations, PGD (ε=0.1, α=0.03) for iterative adversarial attacks, and a Backdoor attack using MZ/numstrings triggers to inject malicious patterns. These poisoned datasets are fed into a detection subsystem, employing a BEAST detector (combining Deep Denoising Autoencoders, Graph Neural Networks, Adversarial Feature Augmentation, and LightGBM) for advanced anomaly detection, alongside a baseline Isolation Forest for comparison, to identify poisoned data effectively. Once detected, the poisoned data enters the correction subsystem, where we use Quantum Tensor Purification (QTP) via a denoising autoencoder to reconstruct clean data, Hyperdimensional Anomaly Isolator (HAI) with random projection to isolate anomalies, Neuro-Symbolic Feature Enforcer (NSFE) to enforce feature bounds, and Incremental Meta-Learning Core (IMLC) for weight optimization, ensuring a robust purification process. The purified data is then used to retrain the XGBoost model, resulting in a secured model resilient to adversarial attacks. This comprehensive approach ensures that we not only detect and mitigate sophisticated poisoning attacks but also maintain model performance and reliability in adversarial environments.
![image](https://github.com/user-attachments/assets/c07a5591-335c-49ad-89ce-ae343c7ca1cc)
![image](https://github.com/user-attachments/assets/5d4710e5-cd68-43a2-9be0-bd974422dcb2)
